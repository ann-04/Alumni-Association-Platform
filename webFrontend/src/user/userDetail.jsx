							...
							<div className='flex flex-col items-center'>
								...
								<p className='text-gray-600 text-center mb-2'>
									{(userDetails?.role === 'STUDENT' || userDetails?.role === 'ALUMNI') &&
										(userDetails?.student?.joiningYear || userDetails?.student?.passingYear) &&
										(userDetails?.student?.joiningYear + ' - ' + userDetails?.student?.passingYear)}
									{userDetails?.role === 'FACULTY' &&
										(userDetails?.faculty?.joiningYear || userDetails?.faculty?.leftYear) &&
										(userDetails?.faculty?.joiningYear + ' - ' + userDetails?.faculty?.leftYear)}
								</p>
					...
					<div className='md:w-2/3 space-y-4'>
						...
						<div className='bg-white rounded-lg shadow-md p-6'>
							...
							{userDetails?.education?.slice().reverse().map((edu) => (
								...
								>
									<div>
										{edu.institute && <h3 className='font-semibold'>{edu.institute}</h3>}
										{edu.degree && <p className='text-gray-600'>{edu.degree}</p>}
										{edu.branch && <p className='text-gray-600'>{edu.branch}</p>}
									...
									<div className='flex items-start justify-between mb-4 last:mb-0 bg-gray-50 rounded-md p-4'>
										<div>
											{userDetails?.student?.institute && (
												<h3 className='font-semibold'>{userDetails.student.institute}</h3>
											)}
											{userDetails?.student?.course && (
												<p className='text-gray-600'>{userDetails.student.course}</p>
											)}
											{userDetails?.student?.branch && (
												<p className='text-gray-600'>{userDetails.student.branch}</p>
											)}
											{(userDetails?.student?.joiningYear || userDetails?.student?.passingYear) && (
												<p className='text-gray-500'>
													{userDetails.student.joiningYear} - {userDetails.student.passingYear} Batch
												</p>
											)}
						...
						<div className='bg-white rounded-lg shadow-md p-6'>
								...
								<div className='flex items-start justify-between mb-4 last:mb-0 bg-gray-50 rounded-md p-4'>
									<div>
										{userDetails?.faculty?.jobTitle && (
											<h3 className='font-semibold'>{userDetails.faculty.jobTitle}</h3>
										)}
										{userDetails?.faculty?.department && (
											<p className='text-gray-600'>{userDetails.faculty.department}</p>
										)}
										{(userDetails?.faculty?.joiningYear || userDetails?.faculty?.leftYear) && (
											<p className='text-gray-500'>
												{userDetails.faculty.joiningYear} - {userDetails.faculty.leftYear}
							...
							{userDetails?.workExperience.map((exp) => (
								...
								>
									<div>
										{exp.jobTitle && <h3 className='font-semibold'>{exp.jobTitle}</h3>}
